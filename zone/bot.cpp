...
void EntityList::BotPickLock(Bot* picker, Client* owner)
{
	Mob* bo = nullptr;
	if (owner && picker->GetOwner() && picker->GetOwner()->CastToClient() == owner)
		bo = owner;

	bool in_owner_group = false;

	Group* g = (picker->GetOwner() && picker->GetOwner()->GetGroup()) ? picker->GetOwner()->GetGroup() : nullptr;
	for (int i = 0; g && i < MAX_GROUP_MEMBERS; ++i) {
		if (g->members[i] == picker) {
			in_owner_group = true;
			break;
		}
	}
	...
	for (auto it = door_list.begin(); it != door_list.end(); ++it) {
		...
		glm::tvec4<float, glm::highp> diff;
		if (bo)
			diff = (bo->GetPosition() - cdoor->GetPosition());
		else
			diff = (picker->GetPosition() - cdoor->GetPosition());

		float curdist = ((diff.x * diff.x) + (diff.y * diff.y));
        if(((diff.z * diff.z) >= 10) || curdist > 130)
            continue;

		const ItemInst* item1 = picker->GetBotItem(MainHands);
		const ItemInst* item2 = picker->GetBotItem(MainPrimary);
        float bonus1 = 0.0f;
        float bonus2 = 0.0f;
		float skill = picker->GetSkill(SkillPickLock);
        if(item1)
		...
		if ((skill + bonus1 + bonus2) >= cdoor->GetLockpick()) {
			cdoor->ForceOpen(picker);
		}
		else {
			if (in_owner_group)
				Bot::BotGroupSay(picker, "I am not skilled enough for this lock");
			else if (bo)
				bo->CastToClient()->Message(MT_Tell, "I am not skilled enough for this lock");
			else
				picker->Say(0, "I am not skilled enough for this lock");
		}
	}
}
...
bool Bot::DyeArmor(int16 slot_id, uint32 rgb, bool all_flag, bool save_flag)
{
	if (all_flag) {
		...
		for (uint8 i = 0; i < MaterialPrimary; ++i) {
		...
		if (mat_slot == _MaterialInvalid || mat_slot >= MaterialPrimary)
			return false;
