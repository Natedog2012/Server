...
#define _EDLEN	200
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			case SpellEffect::SmartHealPercent:
			{
				if (spells[spell_id].limit_value[i] && !PassCastRestriction(spells[spell_id].limit_value[i])) {
					break;
				}
			...
			case SpellEffect::CurrentHP:	// nukes, heals; also regen/dot if a buff
			{
				...
				if (current_limit && current_limit < 0 && effect_value < 0 && caster && caster->IsClient()) {
					...
					auto slot_check = EQ::invslot::slotPrimary;
					if (caster->GetClass() == Class::Ranger) {
						slot_check = EQ::invslot::slotRange;
					}
					auto weapon = caster->CastToClient()->GetInv().GetItem(slot_check);
			...
			case SpellEffect::Leap:
			{
				...
				if (caster && caster->GetTarget()) {
						...
						(caster->CheckLosFN(caster->GetTarget())))
					{
						...
						if(caster->IsClient())
							if (caster->CastToClient()->ClientVersion() >= EQ::versions::ClientVersion::RoF2) {
...
void Mob::DoBuffTic(const Buffs_Struct &buff, int slot, Mob *caster)
{
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		case SpellEffect::HealOverTime: {
			effect_value = CalcSpellEffectValue(buff.spellid, i, buff.casterlevel, buff.instrument_mod);
			if (caster) {
				effect_value = caster->GetActSpellHealing(buff.spellid, effect_value, this, true);
			}
	...
	for (const auto &e : rank.effects) {
		...
		*/
		...
		switch (effect) {
			...
			case SpellEffect::ImprovedDamage3:
