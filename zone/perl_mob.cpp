...
XS(XS_Mob_GetActSpellDamage) {
	dXSARGS;
	if (items < 3 || items > 4)
		Perl_croak(aTHX_ "Usage: Mob::GetActSpellDamage(THIS, uint16 spell_id, int64 value, mob* target)"); // @categories Spells and Disciplines
	{
		Mob *THIS;
		int32  RETVAL;
		dXSTARG;
...
XS(XS_Mob_GetActSpellHealing) {
	dXSARGS;
	if (items < 3 || items > 4)
		Perl_croak(aTHX_ "Usage: Mob::GetActSpellHealing(THIS, uint16 spell_id, int64 value, mob* target)"); // @categories Spells and Disciplines
	{
		Mob *THIS;
		int32  RETVAL;
		dXSTARG;
